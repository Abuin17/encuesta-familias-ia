...
La zona se pregunta por **comunidad o ciudad autónoma** (A7: las 19, todas tratadas igual; se guarda su código INE `01`–`19`). Las cuotas y la anonimización de la exportación agrupan después en las **7 macrozonas NUTS1 de Eurostat** (noroeste, noreste, madrid, centro, este, sur, canarias): la equivalencia está en `ZONA_MACRO` de `lib/encuesta.ts`, y las filas `A7` de la tabla `cuotas` usan esos 7 nombres. Las comunidades salen de la fuente oficial del INE y las macrozonas de la clasificación de Eurostat; `tests/ine-2026.json` y `tests/nuts-2024.json` guardan una copia y los tests comprueban que el código coincide con ellas.
